...
export function buildChainOfCustodyRecord(analysisData = {}, fileProps = {}, extra = {}) {
  ...
  const record = {
    ...
    organization: 'VoiceGuard Digital Audio Forensic Laboratory',
    systemEngine: 'VoiceGuard AI Neural & Steganography Engine v2.4.0',
    ...
    chainOfCustodyTrail: [
      { step: 1, action: 'Evidence Audio Acquisition & Ingestion', timestamp: timestampUtc, operator: 'VoiceGuard Security Gateway', integrityVerified: true },
      ...
      { step: 5, action: 'Steganographic Watermark & Signature Provenance Scan', timestamp: timestampUtc, operator: 'VoiceGuard Watermark Scanner', integrityVerified: true },
    ...
    legalDisclaimer: "This official Forensic Evidence Audit Report is generated by the VoiceGuard AI Security Platform. All SHA-256 evidence digests and digital HMAC signatures are cryptographically sealed to ensure immutability in legal and investigative proceedings."
  };
  ...
  record.digitalSealSignature = crypto.createHmac('sha256', 'VoiceGuard-Legal-Seal-2026-SecretKey').update(signBase).digest('hex');
...
export function generateForensicPdf(reportRecord) {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fillColor('#38bdf8').fontSize(20).text('VOICEGUARD FORENSIC EVIDENCE CERTIFICATE', 36, 20, { bold: true });
      doc.fillColor('#94a3b8').fontSize(9).text(`Digital Audio Forensic Audit Report • Case ID: #${reportRecord.caseId}`, 36, 45);
